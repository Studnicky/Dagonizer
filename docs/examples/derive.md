...
description: 'DAGDeriver derives a DAG topology from OperationContract declarations. Each operation declares what it needs (hardRequired) and what it produces; the deriver wires produces ↔ hardRequired to build the topology automatically.'
seeAlso:
...
- **`DAGDeriver.from(contracts)`.** Accepts an array of `OperationContract` objects. Each contract declares `name`, `hardRequired`, `produces`, `outputs`, and optional `annotations`. The deriver infers edges from the dependency graph.
...
See [Contract-derived flows](../guide/derive) for the full `OperationContract` schema, annotation reference, and output validation.
