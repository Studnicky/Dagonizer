...
## Declarative placement retry

As an alternative to the loop-edge pattern, you can declare retry behaviour directly on the placement using `DAGBuilder.node(..., { retry: { ... } })`. The engine then automatically re-fires the node on throw, applying the configured backoff, without requiring any retry logic inside the node.

```ts
const dag = new DAGBuilder('classify-with-retry', '1.0')
  .node(
    'classify',
    classifyNode,
    { success: 'end', error: 'end-error' },
    {
      retry: {
        ...
        baseDelay:    500,
        jitterFactor: 0.1,
        // Optional: only retry when these output names are in play.
        // Absent means retry on any throw.
        on: ['error'],
      },
    },
  )
...
The `retry` field on a placement maps to `PlacementRetryConfigType`, which extends `RetryPolicyOptionsType` with an optional `on` field. When `on` is present, the engine only retries when the node throw corresponds to one of those named outputs; absent means retry on any throw.

The node itself does not need to be aware of retrying — the placement owns the policy:

```ts
class ClassifyNode extends ScalarNode<MyState, 'success' | 'error'> {
  // No retry logic here. Just execute and return or throw.
  protected override async executeOne(state: MyState) {
    state.result = await callLlm();       // may throw transiently
...
|---------|---------------|
| Loop-edge (`state.withinRetryBudget`) | Retry logic is part of the domain (e.g., the node decides what counts as retriable based on response content). |
| Placement retry (`retry` on the placement) | Retry is an infrastructure concern (network, rate limit, transient LLM errors) independent of node logic. |

Both patterns coexist; a node using `withinRetryBudget` can also sit on a placement with a `retry` config.
