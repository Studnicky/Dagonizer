...
Each strategy run uses `VirtualScheduler` (from `@noocodex/dagonizer/testing`) so retries complete in zero real wall-clock time. Virtual time is advanced programmatically. A `RecordingPolicy` subclass intercepts `getDelay()` to capture the computed delay before forwarding to the scheduler.
...
- **`BackoffStrategy` enum.** Import from `@noocodex/dagonizer`. Pass to `RetryPolicy.from({ strategy: BackoffStrategy.EXPONENTIAL, ... })`.
...
- **`VirtualScheduler`.** Import from `@noocodex/dagonizer/testing`. Replace the real scheduler at dispatcher construction time; call `scheduler.advance(ms)` to drain pending timers without real waits. See [Example: Virtual clock](./virtual-clock) for the full setup.
