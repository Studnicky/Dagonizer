...
description: 'The Cartographer packages its source-normalization child DAGs as a plugin, registers that plugin once, and embeds the exported DAG names from the ingest DAG.'
seeAlso:
  - text: 'Guide: Plugins'
    link: '../guide/plugins'
  ...
  - text: 'Guide: DAGBuilder'
    link: '../guide/builder'
    description: 'the embed() builder alias and embedded DAG state mapping'
...
Plugin-Defined DAGs show the unified interface for plugins and embedded flows. The Cartographer packages source-normalization child DAGs as a plugin, registers that plugin once, and embeds the exported DAG names from the ingest DAG.
...
The [Cartographer](./the-cartographer) packages its format-normalization child DAGs as `normalizeSourcesPlugin`. The browser runner registers the plugin, then registers `ingestSourceDAG`, whose embedded placements reference the plugin-provided DAG names (`normalize-csv`, `normalize-json`, `normalize-ndjson`, `normalize-yaml`).
...
The ingest DAG embeds the plugin-provided child DAG names through the normal builder surface:
...
- **No plugin-specific placement.** `IngestSourceDAG` embeds `normalize-csv` and `normalize-json` exactly like any other child DAG.
- **Scoped package exports.** Applications depend on plugin exports and DAG names, not on plugin internals.
...
- [Guide: DAGBuilder](../guide/builder) - the embed() builder alias and embedded DAG state mapping
