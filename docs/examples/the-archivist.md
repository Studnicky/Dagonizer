...
- **`book-search-scatter`**: extract-query → decide-tools → recall-candidates → build-book-worksets → scatter over `bookWorksets` with `{ dagFrom: 'dagName' }` body (tool-registry dispatch, concurrency 4, `tool-candidate-merge` gather, `any-success` reducer) → rank-candidates → merge-candidates → record-findings → has-citations-gate → recall-past-visits. Used in three intent branches (`on-topic-search`, `author-search`, `similar-search`); one definition, three embedded-DAG placements.
...
| [Example 26: Tool Use](./26-tool-use) | `book-search-scatter` turns tool decisions into `bookWorksets`; each workset selects a registered tool DAG through `dagFrom`. |
