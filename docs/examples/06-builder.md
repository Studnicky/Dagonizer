...
description: 'The Archivist parent DAG authored with the chainable DAGBuilder API. Compile-time route exhaustiveness, sub-DAG placements, parallel nodes, and auto-entrypoint — all in one fluent chain.'
---
...
- **Chainable authoring** — every `.node()`, `.parallel()`, and `.subDAG()` returns `this` for fluent composition. The chain calls `build()` once at the end to produce the plain `DAG` object.
...
- **Sub-DAG placements via `.subDAG()`** — `on-topic-search`, `author-search`, `similar-search`, and `compose-loop` are sub-DAG placements. Each references a registered child DAG by name and declares its `stateMapping.output`.
...
- [Phase 03 · Sub-DAG composition](./03-subflows) — the sub-DAG internals
