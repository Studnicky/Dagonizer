...
description: "ScatterNode generate-and-select pattern: a first-class gather placement folds each clone's produced candidates into parent state before ranking."
seeAlso:
  ...
  - text: 'Example 14: Gather strategies'
    ...
    description: 'gather strategy timing and fan-in barriers'
...
A `ScatterNode` runs a body DAG over every tool workset; each clone produces candidates through a registered tool DAG, and the following `GatherNode` folds those candidates into the parent clone before ranking.
...
- [Example 14: Gather strategies](./14-gather-strategies) - gather strategy timing and fan-in barriers
