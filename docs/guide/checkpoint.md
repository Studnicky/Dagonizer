...
- [Persistence](./persistence) — wire `Checkpoint.persist` / `Checkpoint.recall` to a `CheckpointStore`
- [Cancellation](./cancellation) — abort a flow to produce a non-null cursor worth checkpointing
- [Subclassing State](./subclassing) — override `snapshotData()` / `restoreData()` for domain fields
