---
title: 'ReAct agent: streaming reasoning + provenance recall'
...
at emission time — so a downstream consumer can derive a `wasInformedBy`-style
...
same back-pressure contract every `DagStreamProducer` consumer relies on.
...
enough information for a downstream consumer to tell whose run it belongs to.
