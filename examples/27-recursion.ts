 ...
 * `dagFrom` runtime DAG-name resolution.
 *
 ...
 *   3. Recursive case: EmbeddedDAGNode reads `state.dagName` via `dagFrom`
 *      at runtime, spawns a fresh isolated child frame, seeds it with
...
process.stdout.write('\nLesson: dagFrom reads state.dagName at runtime so the DAG\n');
