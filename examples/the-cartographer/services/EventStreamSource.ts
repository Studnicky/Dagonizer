...
export class EventStreamSource {
  ...
  /**
   * Return an `AsyncIterable<SourcePayload[]>` that yields homogeneous per-type
   * batches of SourcePayload items.
   *
   * Uses `streamTyped` to lazily produce individual payloads, then buffers them
   * per `eventType` and flushes a complete chunk whenever a chunk of `batchSize`
   * items for ONE eventType has accumulated. At the end of the stream, any
   * remaining partial buffers are flushed in the order their eventType was first
   * encountered.
   *
   * Each emitted `SourcePayload[]` is guaranteed to be:
   *   - homogeneous: every item has the same `eventType`
   *   - non-empty
   *   - ≤ batchSize in length
   *
   * Deterministic: calls `streamTyped` with the same LCG-seeded generator, no
   * Date.now / Math.random in the batch-assembly path.
   *
   * @param config     Typed feed configuration.
   * @param count      Optional total event count override (forwarded to streamTyped).
   * @param batchSize  Maximum items per homogeneous batch (default 1000).
   */
  static streamTypedBatches(
    config: EventTypeConfig,
    count?: number,
    batchSize: number = 1000,
  ): AsyncIterable<SourcePayload[]> {
    const cap = Math.max(1, Math.floor(batchSize));
    const source = EventStreamSource.streamTyped(config, count);

    return {
      [Symbol.asyncIterator](): AsyncIterator<SourcePayload[]> {
        const iter = source[Symbol.asyncIterator]();
        // Per-type accumulation buffers. Map preserves first-encounter order.
        const buffers = new Map<string, SourcePayload[]>();
        // Tracks the encounter order of eventTypes so flush order is deterministic.
        const order: string[] = [];
        let done = false;

        async function fillBatch(): Promise<IteratorResult<SourcePayload[]>> {
          // Drain the source until we have a full chunk to yield for any type,
          ...
          while (!done) {
            ...
            const p = step.value;
            let buf = buffers.get(p.eventType);
            if (buf === undefined) {
              buf = [];
              buffers.set(p.eventType, buf);
              order.push(p.eventType);
            }
            buf.push(p);
            if (buf.length >= cap) {
              // Flush a full chunk and keep the remainder in the buffer.
              const chunk = buf.splice(0, cap);
              return { value: chunk, done: false };
            }
          }

          // Source exhausted — flush remaining partial buffers in encounter order.
          for (const et of order) {
            const buf = buffers.get(et);
