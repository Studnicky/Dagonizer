/**
 * SensorReadingEvent: cold-chain telemetry (temperature / humidity / shock).
 *
 * Its `eventType` is pinned to 'sensor-reading'. The `body` carries the shared
 * journey geometry PLUS the three sensor channels (`tempC`, `humidityPct`,
 * `shockG`) — no parcel, customs, delivery, or recipient-PII fields.
 *
...
// #region sensor-reading-event-entity
...
export const SensorReadingEventSchema = {
  ...
  'properties': {
    ...
    'epochMs':      { 'type': 'number' },
    'eventType':    { 'const': 'sensor-reading' },
    ...
    // Per-type body: shared journey geometry + cold-chain sensor channels.
    'body': {
      'type': 'object',
      'required': [
...
export type SensorReadingEvent = FromSchema<typeof SensorReadingEventSchema>;
// #endregion sensor-reading-event-entity
