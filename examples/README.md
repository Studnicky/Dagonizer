...
npx tsx examples/05-embedded-dags.ts     # nested DAG with state mapping
...
npx tsx examples/09-terminals.ts     # TerminalNode: explicit completed/failed endpoints, embeddedDAG routing
npx tsx examples/10-shared-state.ts  # MemoryStore on the services bag + checkpoint round-trip
npx tsx examples/derive.ts           # DAGDeriver: contract-derived DAG + embeddedDAGs annotation (EmbeddedDAGNode placement)
```
