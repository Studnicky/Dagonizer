 ...
 *
 *   1. EventBus — in-process, synchronous, topic-keyed pub/sub.
 *      Publish and subscribe to typed events with automatic cleanup.
 *
...
bus.publish('demo', 'first');
bus.publish('demo', 'second');

// Unsubscribe — subsequent publishes are not delivered.
unsub();
bus.publish('demo', 'after-unsub');
...
sseBus.publish('runs', { 'nodeId': 'fetch', 'event': 'start' });
sseBus.publish('runs', { 'nodeId': 'enrich', 'event': 'start' });
...
const lifecycleBus = new EventBus();
...
process.stdout.write('\n30-progress: done\n');
process.stdout.write('  EventBus: synchronous, topic-keyed, in-process\n');
