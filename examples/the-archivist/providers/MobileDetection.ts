...
export class MobileDetection {
  ...
  static isLikelyMobile(): boolean {
    ...
    const touchPoints = (typeof navigator !== 'undefined' ? navigator.maxTouchPoints : 0) > 1;
    const coarsePointer = window.matchMedia?.('(pointer: coarse)').matches ?? false;
    const narrow = (window.innerWidth ?? 1920) < 900;
  ...
  static setOverride(mode: 'mobile' | 'desktop' | null): void {
    ...
    if (mode === null) localStorage.removeItem('dagonizer-device-override');
    else localStorage.setItem('dagonizer-device-override', mode);
  }
