/**
 * MistralApiAdapter — Mistral AI REST adapter (OpenAI-chat-completions shape).
 *
 * Maps the shared `ChatRequest` to the OpenAI-compatible body Mistral expects:
 *
 *   { model, messages, tools?, tool_choice?, response_format?, … }
 *
 * Response shape is standard OpenAI `chat.completion`. Tool-use via
 * `tools` + `tool_choice`. Structured output via `response_format: { type: 'json_object' }`.
 *
...
export interface MistralApiAdapterOptions {
  readonly apiKey: string;
  readonly model?: string;
  readonly maxAttempts?: number;
}

export class MistralApiAdapter extends BaseAdapter {
  readonly #apiKey: string;
  readonly #model: string;
  ...
  protected async performChat(request: ChatRequest): Promise<ChatResponse> {
    ...
    const timeoutId = setTimeout(() => { controller.abort(new Error('mistral request timeout')); }, TIMEOUT_MS);
    ...
    if (!res.ok) {
      ...
      throw new LlmError(`Mistral REST ${String(res.status)}: ${text}`, classifyHttp(res.status, text));
    }
