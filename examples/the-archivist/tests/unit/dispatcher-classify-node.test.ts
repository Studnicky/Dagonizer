 ...
 *
 * `executeOne` is protected on the base class. A thin public-proxy subclass
 * widens the method to public so tests can invoke node logic directly
 * without the DAG engine. Stub `DispatcherLlmInterface` / `DispatcherIntentInterface`
...
class PublicClassifyMessageNode extends ClassifyMessageNode {
...
describe('ClassifyMessageNode', () => {
  it('trolley switch: humanMode routes to escalate regardless of content', async () => {
    ...
    const node = new PublicClassifyMessageNode(services);
    const result = await node.executeOne(state, CTX);
    assert.equal(result.output, 'escalate');
  ...
  it('empty message routes to off-topic without classification', async () => {
    ...
    const node = new PublicClassifyMessageNode(services);
    const result = await node.executeOne(state, CTX);
    assert.equal(result.output, 'off-topic');
  });
  ...
  it('embedder-first: confident "routine" verdict routes to routine', async () => {
    ...
    const node = new PublicClassifyMessageNode(services);
    const result = await node.executeOne(state, CTX);
    assert.equal(result.output, 'routine');
  });
