...
function makeContext(memory: MemoryStore) {
  return {
...
function seedPriorRun(memory: MemoryStore, runId: string, visitorQuery: string, books: Array<{ isbn: string; title: string }>) {
...
void test('recallCandidates: high-overlap query loads prior shortlisted books', async () => {
  ...
  seedPriorRun(memory, 'prior-run-1', 'existentialism science fiction philosophy', [
  ...
  await recallCandidates.execute(state, makeContext(memory));
  ...
  const titles = state.priorCandidates.map((c) => c.book.title);
...
void test('recallCandidates: unrelated query yields no prior candidates', async () => {
  ...
  seedPriorRun(memory, 'prior-run-2', 'existentialism science fiction philosophy', [
...
void test('recallCandidates: deduplicates books seen in multiple runs', async () => {
  ...
  seedPriorRun(memory, 'prior-run-4a', 'artificial intelligence robots', [
    { isbn: '9999000001', title: 'Do Androids Dream' },
  ]);
  seedPriorRun(memory, 'prior-run-4b', 'robots artificial intelligence singularity', [
    { isbn: '9999000001', title: 'Do Androids Dream' },
  ...
  const isbns = state.priorCandidates.map((c) => c.book.isbn);
...
void test('recallCandidates: salvage path, never throws on corrupted memory entry', async () => {
  ...
  // Should not throw.
  await assert.doesNotReject(() => recallCandidates.execute(state, makeContext(memory)));
  // Book still materialises with fallback title (isbn).
  assert.equal(state.priorCandidates.length, 1);
  assert.equal(state.priorCandidates[0]?.book.isbn, '0000000099');
  assert.equal(state.priorCandidates[0]?.book.title, '0000000099'); // fallback = isbn
});
