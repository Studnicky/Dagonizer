 ...
 *
 * The Archivist greeting must run under the Archivist system directive, but the
 ...
 * the adapter's default Archivist directive. This test exercises the real gate
...
import { BaseAdapter, ChatResponseMessage } from '@studnicky/dagonizer/adapter';
import type {
 ...
 * Constructed with the Archivist system directive, mirroring production, so the
...
class RecordingAdapter extends BaseAdapter {
  ...
  protected async performChat(request: ChatRequestType): Promise<ChatResponseType> {
    ...
    return {
      'message': ChatResponseMessage.create('a visitor question', []),
      'finishReason': 'stop',
