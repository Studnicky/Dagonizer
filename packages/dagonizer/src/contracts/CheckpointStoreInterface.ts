/**
 * CheckpointStoreInterface: adapter contract for persisting `CheckpointData` JSON.
 *
...
export interface CheckpointStoreInterface {
   ...
   */
  save(key: string, json: string, options?: AbortableOptionsType): Promise<void>;
   ...
   */
  load(key: string, options?: AbortableOptionsType): Promise<string | null>;
   ...
   * cancellation; in-process implementations may ignore it.
   */
  delete(key: string, options?: AbortableOptionsType): Promise<void>;
}
