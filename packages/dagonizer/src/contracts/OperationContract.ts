/**
 * OperationContractType: adapter contract for contract-derived flow generation.
 *
 ...
 * `DAGDeriverAnnotationsType.terminals[name]` overrides a specific port.
 *
 ...
 *
 * The `hardRequired` and `produces` fields are defined by `OperationContractFragmentType`.
 * The standalone `OperationContractType` extends the fragment with `name` and `outputs`
 * for use with `DAGDeriver.derive({ contracts })`. When co-locating the contract
 * on a node via `NodeInterface.contract`, use `OperationContractFragmentType` directly;
...
export type OperationContractType = OperationContractFragmentType & {
   ...
   * next derived stage; `DAGDeriverAnnotationsType.terminals` overrides
   * individual ports.
   */
  outputs: string[];
};