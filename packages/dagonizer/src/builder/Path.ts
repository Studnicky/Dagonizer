 ...
 * - Array elements contribute `${number}` and `${number}.${Path<U>}` paths.
 ...
 */
export type Path<T, Depth extends ReadonlyArray<unknown> = []> =
  Depth['length'] extends 8       // depth cap; deep nesting falls back to string
    ? string
    : T extends ReadonlyArray<infer U>
      ? `${number}` | `${number}.${Path<U, [...Depth, 0]>}`
      : T extends object
        ? {
            [K in keyof T & string]:
              T[K] extends object
