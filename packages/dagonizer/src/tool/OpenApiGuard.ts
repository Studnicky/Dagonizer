 ...
 * module-load-compiled `EntityValidatorInterface` — never a hand-written predicate —
 ...
 * instance state. Lives beside `ToolInterface`, `HttpTransport`, and `ToolError` on
...
export class OpenApiGuard {
  ...
  /**
   * Validate `value` against a compiled `EntityValidatorInterface` and return it
   ...
   */
  static assertShape<T>(value: unknown, validator: EntityValidatorInterface<T>, label: string): T {
