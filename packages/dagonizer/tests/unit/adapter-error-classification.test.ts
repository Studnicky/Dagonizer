/**
 * Tests for the ADP-6 discriminated union on ErrorClassification.
 *
 * Verifies:
...
void describe('ErrorClassification discriminated union (ADP-6)', () => {
  void it('retryable classifications have retryAfterMs: null by default', () => {
    ...
    for (const reason of retryableReasons) {
      ...
      assert.equal((c as Extract<ErrorClassification, { retryable: true }>).retryAfterMs, null,
  ...
  void it('classifyHttp 429 with body hint → retryAfterMs: number', () => {
    ...
    assert.equal(c.reason, 'QUOTA_EXHAUSTED');
    assert.equal(c.retryable, true);
    const retryable = c as Extract<ErrorClassification, { retryable: true }>;
