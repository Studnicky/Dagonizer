...
- Release the current workspace fixes and docs consistency updates through the fixed lockstep package set.

## 0.30.0

### Major Changes

- 4639f9b: `Clock`, `RealTimeScheduler`, and `RetryPolicy` now build on `@studnicky/clock`, `@studnicky/scheduler`, and `@studnicky/retry` instead of hand-rolled implementations.
...
- 4639f9b: `ScalarNode.permissiveSchema(outputs)` builds a `{ type: 'object' }` `outputSchema` entry for every listed output name, so nodes that don't need per-port validation write `override get outputSchema() { return ScalarNode.permissiveSchema(this.outputs); }` instead of hand-writing the boilerplate record literal.
...
- 4639f9b: `RetryPolicy.shouldRetry(error, attempt)` consults `DAGError.retryable`. Precedence: an explicit `abortOn` match always stops retrying; an explicit `retryOn` list, when configured, is still the sole authority (a miss stops retrying); when NO `retryOn` filter is configured, a `DAGError` uses its own `error.retryable` field. A non-`DAGError` error with no filters configured is unaffected and still retries.
...
- 4234bc4: `LlmAdapterInterface.chatStream(request, sink)` adds a streaming seam over `StreamSinkInterface<ChatStreamChunkType>`. `BaseAdapter` ships a provider-agnostic buffered default: one full `chat()` call, then a single chunk pushed to the sink. Concrete streaming adapters override `performChatStream` to push incremental deltas as they arrive.
  ...
  New `ChatStreamChunk` entity (`ChatStreamChunkSchema` + `ChatStreamChunkType` + `ChatStreamChunk.create`) carries one incremental text delta from a streaming chat call.

  New `ReasoningStep` entity (`ReasoningStepSchema` + `ReasoningStepType` + `ReasoningStep.create`) models one step — `thought` / `action` / `observation` / `final` — of an agent's reasoning trace as a discriminated union.

  New `ReasoningTraceItem` entity (`ReasoningTraceItemSchema` + `ReasoningTraceItemType` + `ReasoningTraceItem.create`) pairs a `ReasoningStepType` with a monotonic `ordinal`, so a streamed step is self-describing — a downstream consumer can derive a `wasInformedBy`-style chain from `ordinal - 1` with no cross-item state.
...
- `DagLoggerInterface` (`src/ObservedDag.ts`) uses `@studnicky/logger`'s structured call shape instead of plain strings: `trace(body)`, `debug(body)`, `info(body)` take a `LogBodyDataType`, and `error(fault)` takes a `LogFaultDataType`. A real `@studnicky/logger` `Logger` instance satisfies `DagLoggerInterface` directly with no adapter. `ObservedDag`'s lifecycle hooks (`onFlowStart`, `onFlowEnd`, `onNodeStart`, `onNodeEnd`, `onError`, `onPhaseEnter`, `onPhaseExit`) emit direct structured data with events (`dag.flow`, `dag.node`, `dag.phase`), lifecycle status (`in_progress`/`complete`/`failed`), a human-readable `message`, and structured `context` (`dagName`, `nodeName`, `placementPath`, `outcome`, `phase`, `placementName`, `output`).
...
- `ScatterWorkerPool` now bounds scatter-node concurrency with `@studnicky/concurrency`'s `Semaphore` instead of a hand-rolled slot counter. The pool creates one `Semaphore` (`Semaphore.create({ 'permits': concurrencyLimit })`) in its constructor and drives the pull loop by `acquire()`ing a permit before each pull and calling the returned release function once an item's execute+ack cycle settles; the internal `#activeWorkers` counter and `#slotResolve`/`#waitForSlot`/`#releaseSlot` methods are removed. The pull loop re-checks accumulated worker errors and the abort signal immediately after each `acquire()` resolves — not only at loop entry — so a worker error recorded while an iteration was queued for a permit is observed before another item is pulled. `ScatterWorkerPool`'s public constructor and `drain()`/`errors` surface are unchanged.
...
- `RetryPolicy.shouldRetry(error, attempt)` consults `DAGError.retryable`, so the framework's three failure-handling mechanisms (`RetryPolicy`'s `retryOn`/`abortOn` filters, `BaseAdapter`'s opt-in circuit breaker/token bucket, and `DAGError.retryable`) form one coherent precedence. Order: an explicit `abortOn` match always stops retrying (consumer-authoritative override, even against a `DAGError` that self-reports `retryable: true`); a configured `retryOn` list is the sole authority when present (a miss stops retrying); only when NO `retryOn` filter is configured does a `DAGError` use its own `error.retryable` field. A non-`DAGError` error with no filters configured is unaffected and still retries.
...
- 6bdafa4: `BaseAdapter.selectChatModel()` prefers a fully-local model when auto-selecting the cheapest chat model. A cloud-routed model (e.g. Ollama's `:cloud`/`-cloud` tags) reports a near-zero local footprint, so the cheapest selector picks from the non-`cloud` models when any exist and picks a cloud model only when no local chat model is installed. An explicit in-catalogue `preferred` model still wins regardless of its `cloud` flag.
...
- **`BaseAdapter` default-system-prompt seam.** `BaseAdapterOptionsType` (exported from `./adapter`) extends the core options with `systemPrompt`: a consumer-supplied default directive the base injects as the leading message of any chat request that carries no system message of its own — never overriding an explicit system turn, never producing a second one, no-op when unset (`''`). The engine owns no directive; a consumer frames role/format/language once at construction and the adapter stays backend plumbing. Leading position is load-bearing: the on-device backends (Chrome Prompt API, MLC WebLLM) reject a system message at any non-zero index. `OpenAiCompatibleAdapter` and its `groq`/`cerebras`/`mistral`/`openRouter` static factories accept and forward the same `systemPrompt` option.
...
- 54252c9: DAGDocument.load() accepts an optional overrides object merged
...
- 4d55c20: TypeScript devDependency bumped from ^5.6.0 to ^6.0.3 across all packages
  ...
  - `docs/tsconfig.json`: removed `baseUrl: "."` (unsupported in TS 6, reported as
    TS5101; unused under `moduleResolution: Bundler` with no path remaps).
  - CSS ambient module declarations cover side-effect imports.
...
- a79da55: Adds five framework-tooling surfaces that consumers previously hand-rolled:
  ...
  - **`Agent`** (`./patterns`) — assembles the eight-node agent tool-calling
  ...
  - **`LlmAdapterCascade.create`** (`./adapter`) — assembles a configured
...
- a79da55: Services constructor-DI, typed metadata reads, store typed reads, and a repo-wide
  ...
  - **Typed metadata reads via `state.getter` (`MetadataGetter`).** `NodeStateBase.getMetadata(key)`
    ...
    `state.getter` — a `MetadataGetter` with `string(key, defaultValue?)`, `number`,
...
- 59a763d: Tech-debt remediation — a coherent breaking release across the whole monorepo (all
  ...
  **Naming convention (types-are-data / interfaces-are-contracts).** Enforced by ported
  ...
  (`DAGBuilder.derive`, `ScatterOptions.resolve`, `NodeContext.create`,
...
- `BaseAdapter.formatToolResult(message)` (`./adapter`): static that formats a `tool`-role `ChatMessage` as `[tool <name> result] <content>`, using `unknown` for a blank `toolName`. Text-only adapters that flatten tool results into the prompt share this one source of the string.
...
- **Naming: domain-class verbs, grouped by entity (semver-major).** Public static factories and override seams use direct domain verbs on their owning classes. `RegistryModuleInterface.instantiate` is the registry-module entry point. Consumer override seams use direct verbs (`CytoscapeGraph.composeElements`, `DagContainerBase.composeEntry`). Node-context construction lives on its own entity as `NodeContext.create(dagName, nodeName, signal, services)`, decoupling context assembly from the engine class. `StateMapper.cloneChild` owns child-state creation. Engine-internal helpers use direct domain verbs (`composeResult`, `composeGatherExecution`, `relayFor`, `#composeBody`, `#decodeResponse`, `#decodeJson`, `decodeJson`, `composeNodeData`, `composeUpstreamProducers`).
...
- d5a95ea: DAG containment, cross-host hand-off, crash-safe transport, and a full audit-and-harden pass.
  ...
  **Breaking — opinionated surface**
  ...
  - Removed duplicate names: `SchedulerHandle`, `PhaseNodePlacementInterface`, `TerminalNodePlacementInterface`, `StateRestoreFnType`. `AdapterBase` → `BaseAdapterCore`.
...
### Breaking — removed duplicate names

One canonical name per symbol; duplicate names and placement-interface re-exports are removed.
...
- **`NodeError` and `NodeOutput`** are exported from the root `.` and `./entities` surfaces.
...
- **`NodeError.create(code, message, operation, recoverable, timestamp, options?)`** — required args are positional.
...
- **Sub-DAG cycle detection removed from `DAGValidator`.** The append-only registry combined with the rule that every `EmbeddedDAGNode`/`ScatterNode` body must reference an already-registered DAG makes the reference graph structurally acyclic. Explicit cycle detection is unnecessary and is no longer present.
...
- **`NodeError.create`** constructs a complete `NodeError`, filling `context: {}` when absent. Ships through `.` and `./entities`.
- **`NodeOutput.create`** constructs outputs with `errors: []` as the default. Read `.errors` directly.
...
### Breaking — direct contract surface
...
- **`RealTimeClockProvider` requires `performance.now()`** (Node 24+ / modern browsers); `Date.now()` is not used.
...
- **`BridgeMessage`** (`./entities`): static factory for `BridgeMessage` values. `BridgeMessage.create({ code, message })` builds a channel-scoped error message (`correlationId: null`) for init failures, transport setup errors, and invalid message receipts at the channel boundary.
...
- 8b47957: viz: ship a subclassable `CytoscapeGraph` factory, make the visualizer opt-in, and fix self-loop node rendering.
  ...
  - **`cytoscape` and `@dagrejs/dagre` are optional peer dependencies.** The visualizer is opt-in: consumers who do not import `./viz` install neither. External consumers of the cytoscape renderer receive `@dagrejs/dagre` through the declared peer contract.
  ...
  - **Fixed:** DAG nodes carrying a self-loop edge (a `retry` route to self) render with explicit numeric dimensions and a concrete monospace font stack, with a post-layout visibility sweep as a guard.
...
- b5b931f: Audit-driven cleanup across the monorepo (performance, V8 shape, consistency) — every confirmed and advisory finding addressed.
  ...
  - consistency: wire-format helpers in `OpenAiCompatibleAdapter` are private methods (no freestanding `toX`/`parseX` functions); removed the duplicate `SearchTool` name from `./patterns` (use canonical `Tool` from `./tool`).
...
- a338274: Add `WellFormedValidator` (`./validation`): an opt-in authoring lint that flags invalid DAG shapes the structural Ajv schema cannot express — bare `null` flow-ends (route to a canonical `TerminalNode` instead), dangling output targets, and malformed scatter/embedded/terminal placements. It returns human-readable violations and is NOT wired into the permissive runtime `registerDAG` (where `null` routes remain a supported natural-end). The repo's flagship example DAGs are gated against it via a new `lint:dags` CI step.
