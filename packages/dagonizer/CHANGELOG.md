...
- **Agent-flow template-method node family (`./patterns` subpath).** Nine abstract base classes replace the deleted callback-based `AgentFlowNodes`: `BuildChatRequestNode`, `CallModelNode`, `NormalizeResponseNode`, `DecodeTextToolCallsNode`, `AppendAssistantNode`, `NormalizeToolCallsNode`, `BuildToolWorksetsNode`, `CollectToolResultsNode`. Each base carries `protected abstract` template methods; consumer subclasses override them — zero callbacks, zero function injection. `CallModelNode` receives its `LlmAdapterInterface` through its constructor (dependency injection) and reads it as `this.llm`; a consumer subclass passes the adapter to `super(...)`. `ToolCallScatterItemType` stamps each normalized tool call with `dagName: 'tool:<name>'` so a scatter placement using `{ dagFrom: 'dagName' }` resolves the body DAG from `ToolRegistry` at runtime. `ToolInvokeNode` gains a scatter-path fallback: when `state.input` is empty, it reads the tool-call arguments from state metadata so scatter-dispatched tools receive their call arguments without parent-state coupling.

### Changed

- **Breaking: services are injected into nodes, not threaded through the dispatcher (semver-major).** The `TServices` generic, the `context.services` field, and the `Dagonizer` `services` constructor option are removed. A node that needs an external dependency (an LLM adapter, a store, a tool, a triple-store) receives it through its own constructor and holds it as an instance field; it is registered with that dependency (`dispatcher.registerNode(new FetchNode(db))`). `Dagonizer` is now `Dagonizer<TState>` (one type parameter); `NodeInterface<TState, TOutput>`, `ScalarNode<TState, TOutput>`, `MonadicNode<TState, TOutput>`, and `NodeContextType` are non-generic over services; `NodeContextType` carries only `signal`, `dagName`, `nodeName`, `validateOutputs`, and `outputSchemaValidator`. `AgentServicesType` is removed. The pattern packages follow suit: `GraphNode(memory)`, `LlmDispatchNode(llm)`, and `ScoutNode(tool)` take their dependency as a constructor argument. **Migration:** drop the `<TServices>` type argument and the `{ services }` option; give each node a constructor that accepts the dependencies it read from `context.services`, store them as fields, and pass them when constructing the node for registration. This is the framework's DI / class-extension model — the dispatcher no longer carries an ambient services record, and no generic-default cast is needed to materialise one.

- **Breaking: `TState` generic removed from engine internals (semver-major).** The engine threads state through `NodeStateInterface` uniformly across all internal modules. `DagTaskInterface` and `DagContainerInterface` are non-generic; `GatherExecutionType<TItem>` carries `state` as `NodeStateInterface`. The public boundary (`Dagonizer<TState>`, `execute`, `resume`, `Execution<TState>`, `NodeResultType<TState>`, `ExecutionResultType<TState>`) keeps `TState`. This reflects the engine's actual design: embedded/scatter child DAGs run on their own heterogeneous state classes — each child state implements `NodeStateInterface` but is not `TState`. The prior `TState` threading was a fiction that forced casts at every containment boundary. Consumers who pass a concrete `TState` generic to `DagContainerInterface` or `DagTaskInterface` remove that type argument; all other public-API call sites are source-compatible.

### Added

- **Typed metadata reads via `state.getter` (`MetadataGetter`).** `NodeStateBase.getMetadata(key)` returns `unknown` (the metadata store holds arbitrary JSON), so every state exposes `state.getter` — a `MetadataGetter` with `string(key, fallback?)`, `number`, `boolean`, `stringArray`, and `numberArray` that narrow each read to a concrete type with a required default, cast-free. Use it instead of narrowing `getMetadata` at the call site. `MetadataGetter` ships on the root barrel; `MetadataReadableInterface` (its minimal read contract) ships on `./contracts`.

### Removed

- **Internal `as` casts eliminated across the engine.** `StateAccessorInterface.get` returns `unknown` (was `get<T>(): T | null` with an internal `as T`) and callers narrow; `Validator`'s compiled functions are typed as `ValidateFunction<T>` so validation narrows without a cast; `EventBus.publish`/`subscribe` operate on an `unknown` payload (subscribers narrow); platform timer/clock access, dotted-path traversal, and the Mermaid/dagre viz boundaries narrow through `noun.is` type-guards and enriched stub types rather than assertions. **Migration:** replace `accessor.get<T>(state, path)` with `accessor.get(state, path)` plus a narrowing check (`Array.isArray`, `typeof`, or a guard); replace `bus.subscribe<T>(topic, fn)` with `bus.subscribe(topic, fn)` and narrow `event.payload`.
