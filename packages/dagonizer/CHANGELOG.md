...
- `GatherRecord<TState>` and `GatherExecution<TState>` (`./contracts`) gain a defaulted `TItem = unknown` generic; `GatherRecord.item` is `TItem` rather than a permanent `unknown`. `RegistryBundleInterface` and `RegistryModuleInterface` (`./contracts`) gain a defaulted `TServices = unknown` generic threaded through `services` and `createBundle`'s return. Both defaults keep every existing call site source-compatible.
