...
- Add a per-request `timeoutMs` option (default 60s) enforced as a deadline around the in-browser MLC generation; an expired deadline (or an aborted request signal) surfaces as a `TIMEOUT` classification so a cascade falls through rather than hanging. The underlying WebGPU run is not cancellable, so the deadline frees the caller without cancelling compute.
