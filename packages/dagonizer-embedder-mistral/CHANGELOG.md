...
- **Mandatory `Type` suffix on the embed-response entity type (semver-major rename).** The `FromSchema`-derived response type is `MistralEmbedResponseType` (was `MistralEmbedResponse`); the `MistralEmbedResponseSchema` const and `MistralEmbedResponseValidator` keep their names. The renamed type ships from the package root. Type-only; behavior is unchanged.
