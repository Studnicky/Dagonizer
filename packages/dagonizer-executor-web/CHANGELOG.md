...
- `WebSystemInfo`: `SystemInfoInterface` implementation using injected `hardwareConcurrency` and `crossOriginIsolated` probes. Implements the quadrascope clamp formula: `clamp(hardwareConcurrency − mainThreadReservation, minimumWorkerCount, maximumWorkers)`.
...
- Unit tests: `post-message-channel.test.ts` (round-trip, invalid-payload, close semantics, structuredClone isolation), `web-system-info.test.ts` (clamp math, safe defaults, fuzz sample), `conformance.test.ts` (full `DagConformance.laws()` Laws 1–6 and 9 via in-process `FakeWorker` driving a `DagHost`, pool-size queuing, `destroy()` terminate coverage).
