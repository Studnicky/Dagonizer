...
function fakeOs(parallelism: number, freemem: number = 4 * 1024 * 1024 * 1024): {
...
void describe('NodeSystemInfo.recommendedWorkerCount — clamp math', () => {
  void it('returns parallelism − mainThreadReservation when within bounds', () => {
    const info = new NodeSystemInfo({ 'os': fakeOs(4) });
    assert.strictEqual(info.recommendedWorkerCount(config({ 'maximumWorkers': 8 })), 3);
  });

  void it('clamps to maximumWorkers when parallelism is large', () => {
    const info = new NodeSystemInfo({ 'os': fakeOs(32) });
    assert.strictEqual(info.recommendedWorkerCount(config({ 'maximumWorkers': 4 })), 4);
  });

  void it('clamps to fallbackWorkerCount when parallelism ≤ mainThreadReservation', () => {
    const info = new NodeSystemInfo({ 'os': fakeOs(1) });
    assert.strictEqual(info.recommendedWorkerCount(config({ 'fallbackWorkerCount': 1 })), 1);
  });

  void it('uses fallbackWorkerCount = 2 when base is negative', () => {
    const info = new NodeSystemInfo({ 'os': fakeOs(0) });
    assert.strictEqual(
      info.recommendedWorkerCount(config({ 'fallbackWorkerCount': 2 })),
      2,
    );
  });

  void it('exact parallelism − reservation = maximumWorkers returns maximumWorkers', () => {
    const info = new NodeSystemInfo({ 'os': fakeOs(5) });
    assert.strictEqual(info.recommendedWorkerCount(config({ 'maximumWorkers': 4 })), 4);
  });
});
...
void describe('NodeSystemInfo.recommendedWorkerCount — memory clamping', () => {
  void it('further clamps by freemem / memoryPerWorkerBytes', () => {
    ...
    const info = new NodeSystemInfo({ 'os': fakeOs(9, freemem) });
  ...
  void it('does not reduce below fallbackWorkerCount from memory clamping', () => {
    ...
    const info = new NodeSystemInfo({ 'os': fakeOs(4, freemem) });
    const result = info.recommendedWorkerCount(config({
      'memoryPerWorkerBytes': perWorker,
      'fallbackWorkerCount': 1,
    }));
  ...
  void it('null memoryPerWorkerBytes skips memory clamping', () => {
    const info = new NodeSystemInfo({ 'os': fakeOs(4) });
    const result = info.recommendedWorkerCount(config({ 'memoryPerWorkerBytes': null }));
