...
- **HITL park-and-correlate primitive.** A node routes to the reserved `'parked'` output to pause execution for human input. The engine transitions the lifecycle to the new `'awaiting-input'` variant (non-terminal), sets `result.cursor`, and populates `result.parked: ParkedType` with a `correlationKey` (written to state metadata by the node), `cursor`, and `dagName`. The caller captures a checkpoint and calls `dispatcher.resume()` when the decision arrives; the scheduler resets the lifecycle on resume exactly as it does for crash-recovery. `DAGLifecycleMachine` gains the `'awaiting-input'` variant, `park` event, and `isParked()` predicate. `NodeStateBase` and `NodeStateInterface` gain `park(correlationKey)` and a `parked` getter. `ExecutionResultType` gains `parked: ParkedType | null`. `Validator.parked` is registered. The DAG registration validator skips the reserved `'parked'` output in the routing-completeness check so placements need not (and should not) map it. `ParkedSchema` and `ParkedType` export from `./entities` and the root barrel.
- **Plugin loader: `PluginInterface` + `Dagonizer.registerPlugin(plugin)`.** A plugin package implements the one-method `PluginInterface` (`register(dispatcher: PluginReceiverType): void`) and installs its nodes and DAGs via the narrow `PluginReceiverType` seam (only `registerBundle` is visible). The dispatcher registers a plugin with a single `registerPlugin(plugin)` call; registration order matches call order. `PluginInterface` and `PluginReceiverType` export from the root barrel, `./contracts`, and `./types`.
- **`PluginDiscovery` static DAG-walker** (`@studnicky/dagonizer/plugin`). New `./plugin` subpath. `PluginDiscovery.referencedDagNames(dag)` collects all literal `dag` body references from a DAG's placement graph. `PluginDiscovery.walk(dag, registry)` performs a breadth-first traversal of the full reachable sub-DAG forest. `dagFrom` (runtime-resolved) bodies are excluded.
- **Multi-observer mux: `DagonizerOptionsType.observers` + `DispatcherObserverType`.** The `observers` option on `DagonizerOptionsType` accepts a `ReadonlyArray<DispatcherObserverType>`. Each record's callbacks mux into the corresponding lifecycle hook (all seven: `onFlowStart`, `onFlowEnd`, `onNodeStart`, `onNodeEnd`, `onError`, `onPhaseEnter`, `onPhaseExit`) in array order, after any subclass override. `DispatcherObserverType` exports from the root barrel and `./types`. Provides a first-class alternative to subclassing for per-turn-rebuilt dispatchers.
