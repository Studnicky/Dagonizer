...
- Archivist demo: molecular DAG composition. Two reusable sub-DAGs shipped as components — `BookSearchFanoutDAG` (4-source parallel scout cluster + rank + merge + record + citations-gate, used in three intent branches) and `ComposeRetryLoopDAG` (recall + compose + validate with bounded retry, the shared response terminus). Each sub-DAG exports a `register{Name}Nodes(dispatcher)` helper so consumers import the cluster and register its nodes in one call. `archivistDAG` shrinks from 348 → 236 lines and now reads as a composition of named sub-DAGs. `CytoscapeRenderer` receives the sub-DAG registry and expands each placement inline — no opaque boxes.
