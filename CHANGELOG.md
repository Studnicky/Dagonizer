...
- **`TState` eliminated from all engine-internal execution types. ⚠ BREAKING for implementors of engine contracts.** The `TState` type parameter is removed from every contract and class that does not need it at the public boundary. Types affected: `DagContainerInterface` (now non-generic), `DagTaskInterface<TServices>` (was `DagTaskInterface<TState, TServices>`), `ScatterItemResultType`, `ScatterItemBatchResultType`, `ScatterPoolDriverInterface`, `ReservoirDriverInterface`, `ScatterWorkerPool`, `ReservoirBuffer`, `DagContainerBase<TWorker>` (was `DagContainerBase<TState, TWorker>`), `RunNodeResultType`, `RunNodesBatchType`, `ScatterRunContextType`, `ScatterDispatchAdapterInterface`, `ScatterDispatchSourceInterface`, `GatherRecordType` (state field is `NodeStateInterface`; `TItem` remains). The new `ChildStateFactoryType` (`(parent: NodeStateInterface) => NodeStateInterface`) and `ChildStateFactory.cloneParent` default make `registerDAG(dag, factory?)` accept isolation factories for child-state construction. `dagFrom` (a dotted state path whose resolved string names the dag at runtime) is now a valid `ScatterNode` body variant alongside `node` and `dag`; unregistered resolved names route to `error`. `DagonizerOptionsType` drops its `TState` parameter. **Migration:** subclasses of `DagContainerBase` drop the first type argument (`DagContainerBase<TState, Worker>` → `DagContainerBase<Worker>`); implementations of `DagContainerInterface` remove the type argument; implementations of `DagTaskInterface` replace the two-arg form with the one-arg form (`DagTaskInterface<TServices>`); direct uses of the removed types listed above similarly drop their state type argument.
